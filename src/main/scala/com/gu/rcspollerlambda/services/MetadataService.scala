...
object MetadataService extends Logging {
  def pushRightsUpdates(
      wsClient: StandaloneAhcWSClient,
      rcsUpdates: List[(String, Json)]
  ): Either[LambdaError, String] = {
    logger.info(
      s"Sending ${rcsUpdates.length} json RCS update(s) to the Metadata service..."
    )
      ...
      try {
        rcsUpdates
          .filter { case (id, _) =>
            ...
            if (!validId)
              logger.warn(
                s"Dropping rights update as $id is not a legitimate grid ID"
              )
            validId
          }
          ...
          case Nil =>
            Right(
              s"${rcsUpdates.length} writes successfully written to Syndication service"
            )
          case errors =>
            Left(
              MetadataServicePublishError(
                "Error(s) writing to Syndication service",
                errors.mkString("\n")
              )
            )
        }
        ...
        case t: Throwable =>
          Left(
            MetadataServicePublishError(
              "Unable to publish to metadata service",
              t.toString
            )
          )
      }
