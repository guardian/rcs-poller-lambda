...
object HTTP extends Logging {
  ...
  def getXml(
    ...
    logger.info(
      s"Fetching XML from $url?lastid=$lastid&subscribername=$subscriberName"
    )
    try {
      Await.result(
        wsClient
      ...
      case e: Throwable =>
        ...
        Left(
          RCSError(
            e.getClass.getCanonicalName + " / " + fullStackTraceWriter.toString
          )
        )
    }
  }
          ...
          .map { result =>
            ...
            result.status match {
              ...
              case _   =>
                Left(
                  MetadataServicePublishError(
                    s"HTTP error ${result.status.toString}",
                    result.body
                  )
                )
            }
